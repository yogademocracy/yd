...
/**
 * Update custom PLI attributes such as weight
 *
 * @param {productLineItem} productLineItem - productLineItem
 * @param {number} quantity - the number of products to the cart
 */
function updatePLIAttibutes(product, productLineItem, quantity) {
    var Transaction = require('dw/system/Transaction');

    if (!empty(product.custom.weightGrams)) {
...
base.addProductToCart = function (currentBasket, productId, quantity, childProducts, options) {
    ...
    if (productInCart) {
        ...
        if (availableToSell >= quantityToSet || perpetual) {
            ...
            updatePLIAttibutes(product, productInCart, quantityToSet);
        } else {
            result.error = true;
        ...
        // Yoga democracy modification
        updatePLIAttibutes(product, productLineItem, quantity);
    }
...
// Yoga democracy modification
base.updatePLIAttibutes = updatePLIAttibutes;
