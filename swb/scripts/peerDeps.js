...
exec('npm -v', function (error, stdout, stderr) {
  ...
  if (npmVersion <= 3 || npmVersion > 7) {
    console.log('[SWB] Doing nothing... Peer dependencies will be installed by npm automatically');
    return;
  }
  exec(
    `\
    ...
    (err) => {
      ...
      console.log('[SWB] Peer dependencies installed successfully. Ignore peer dependencies install warn from npm');
    }
  );
});
