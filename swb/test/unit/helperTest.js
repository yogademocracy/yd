...
describe('buildRuleSet()', () => {
  ...
  it('should include all relevant paths for node_modules', () => {
    ...
    let expectedLoadPaths = [];
    rules.map((rule) => {
      rule.use.map((taskRunner) => {
        availableTaskRunners.push(taskRunner.loader);
        if (taskRunner.options?.sassOptions?.loadPaths) {
          expectedLoadPaths = taskRunner.options?.sassOptions?.loadPaths;
        }
      });
    });
    ...
    let loadPathsToCheck = sfraBuilderConfig.cartridges.length * 2 + 2;
    assert.strictEqual(loadPathsToCheck, expectedLoadPaths.length);
