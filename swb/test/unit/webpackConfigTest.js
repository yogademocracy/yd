...
describe('bundleCartridge()', () => {
  it('should use TerserPlugin', () => {
    ...
    env.dev = false;
    env.useLinter = false;
    const bundledConfig = WebpackConfig.bundleCartridge(env, path.resolve(__dirname, '../fixtures'), 'js');
    assert(bundledConfig.optimization.minimizer[0].constructor.name.toString() === 'TerserPlugin');
  });
  ...
  it('should use eslint when linter is set to true', () => {
    ...
    const bundledConfig = WebpackConfig.bundleCartridge(env, path.resolve(__dirname, '../fixtures'), 'js');
    let hasEslint = bundledConfig.plugins.some((plugin) => plugin.constructor.name.toString() === 'ESLintWebpackPlugin');
  ...
  it('should not use eslint when linter is set to false', () => {
    let env = {};
    const bundledConfig = WebpackConfig.bundleCartridge(env, path.resolve(__dirname, '../fixtures'), 'js');
    const noEslint = bundledConfig.plugins.every((plugin) => plugin.constructor.name.toString() != 'ESLintWebpackPlugin');
    assert.isTrue(noEslint);
  });

  it('should use stylelint when linter is set to true', () => {
    let env = { useLinter: true };
    const bundledConfig = WebpackConfig.bundleCartridge(env, path.resolve(__dirname, '../fixtures'), 'scss');

    const hasStyleLint = bundledConfig.plugins.some((plugin) => plugin.constructor.name.toString() === 'StylelintWebpackPlugin');
  ...
  it('should not use stylelint when linter is set to false', () => {
    let env = {};
    const bundledConfig = WebpackConfig.bundleCartridge(env, path.resolve(__dirname, '../fixtures'), 'scss');
    const noStyleLint = bundledConfig.plugins.every((plugin) => plugin.constructor.name.toString() != 'StylelintWebpackPlugin');
    assert.isTrue(noStyleLint);
  });

  it('should use livereload when env livereload is set to true', () => {
    let env = { livereload: true };
    const bundledConfigJs = WebpackConfig.bundleCartridge(env, path.resolve(__dirname, '../fixtures'), 'js');
    const bundledConfigScss = WebpackConfig.bundleCartridge(env, path.resolve(__dirname, '../fixtures'), 'scss');

    assert.isTrue(bundledConfigJs.plugins.some((plugin) => plugin.constructor.name.toString() === 'LiveReloadPlugin'));
    assert.isTrue(bundledConfigScss.plugins.some((plugin) => plugin.constructor.name.toString() === 'LiveReloadPlugin'));
  });
  ...
  it('should not use livereload when livereload is set to false', () => {
    ...
    assert.isTrue(bundledConfigJs.plugins.every((plugin) => plugin.constructor.name.toString() != 'LiveReloadPlugin'));
    assert.isTrue(bundledConfigScss.plugins.every((plugin) => plugin.constructor.name.toString() != 'LiveReloadPlugin'));
  });

  it('should return null on non-existings folders', () => {
    let env = {};
    env.dev = true;
    const bundledConfig = WebpackConfig.bundleCartridge(env, path.resolve(__dirname, '../doesNotEists'), 'js');
  ...
  it('should createOutputfiles for js/css', () => {
    ...
    exec('npm run test:fixture', (err) => {
      ...
      try {
        folders = fs.readdirSync('./test/fixtures/cartridge/static/default/css');
        cssFiles = fs.readdirSync('./test/fixtures/cartridge/static/default/css');
