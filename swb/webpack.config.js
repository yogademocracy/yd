...
let sfraBuilderConfig = process.env.npm_lifecycle_script.indexOf('testRunner') === -1
...
class WebpackBundle {
    ...
    static scanEntryPoints(cartridge, fileType) {
        ...
        const srcSCSSPath = path.join(srcPath, '*', 'scss', '**', '*.scss').replaceAll('\\', '/');
        const srcJSPath = path.join(srcPath, '*', 'js', '*.js').replaceAll('\\', '/');
        const srcJSXPath = path.join(srcPath, '*', 'js', '*.jsx').replaceAll('\\', '/');
        let files = {};
        ...
        if (fileType === 'scss') {
            glob
                ...
                .forEach((source) => {
                    ...
                    const outputFile = path.join(sourceRelativePath, sourceName).split(path.sep).join(path.posix.sep).replace('.scss', '.scssjs'); // Webpack always create the output file
        ...
        if (fileType === 'js') {
            glob
                ...
                .filter((source) => {
                    ...
                    if (sfraBuilderConfig.excludeJS && sfraBuilderConfig.excludeJS[cartridge] && sfraBuilderConfig.excludeJS[cartridge].indexOf(path.basename(source)) !== -1) {
        ...
        if (fileType === 'jsx') {
            ...
            glob
                ...
                .forEach((source) => {
                    const sourceRelativePath = path.dirname(path.relative(srcPath, source));
                    const sourceName = path.basename(source);
                    const outputFile = path.join(sourceRelativePath.replace('jsx', 'js'), sourceName.replace('.jsx', '.js')).split(path.sep).join(path.posix.sep);
                    files[outputFile] = source;
                });
        }
        return files;
        ...
        if ((fileType === 'js' || fileType === 'jsx') && env.useLinter) {
            plugins.push(
                new ESLintWebpackPlugin({
                    ...
                    exclude: ['./node_modules', './cartridges/app_storefront_base'],
                    fix: sfraBuilderConfig.lintConfig && sfraBuilderConfig.lintConfig.eslintFix
                })
            );
            if (env.wba) {
                let cartridgeName = cartridge.substring(cartridge.lastIndexOf('/') + 1);
                plugins.push(
                    new BundleAnalyzerPlugin({
                        analyzerMode: 'static',
                        reportFilename: `wba-report-${cartridgeName}.html`,
                        reportTitle: `Cartridge: ${cartridgeName}`
                    })
                );
            }
        }
    ...
    static buildDynamicAliases(cartridgeAliases, fileType) {
        ...
        aliases.bootstrap = path.resolve(process.env.PWD, './node_modules/bootstrap');
        aliases.lodash = path.resolve(process.env.PWD, './node_modules/lodash');
        return aliases;
    }
    ...
    static bundleCartridge(env = {}, cartridge, fileType) {
        ...
        if (Object.keys(sfraBuilderConfig.aliasConfig).length === 0 || Object.keys(sfraBuilderConfig.aliasConfig.alias).length === 0) {
...
/**
 * ClearConsolePlugin: Clears the console before each watch-run.
 */
class ClearConsolePlugin {
    apply(compiler) {
        compiler.hooks.watchRun.tap('ClearConsolePlugin', () => {
            // Clears the terminal in a cross-platform way (works on Windows)
